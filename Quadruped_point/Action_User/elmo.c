...
void MotorOn(CAN_TypeDef* CANx, uint8_t ElmoNum)
{
	...
	while((CAN_TransmitStatus(CANx, mbox)!= CAN_TxStatus_Ok))
	{
		...
		if(timeout > 3000)
		{
			if(CANx == CAN1)
			{
				USART_OUT(UART4,(uint8_t*)"can1 %d send err\r\n",ElmoNum);
			}
			else if(CANx == CAN2)
			{
				USART_OUT(UART4,(uint8_t*)"can2 %d send err\r\n",ElmoNum);
			}
			break;
		}
		timeout = 0;
...
* @author ACTION
*/
void VelCrl(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t vel1,int32_t vel2)
{
	SetJoggingVel(CANx, ElmoNum, vel1,vel2);
...
* @author ACTION
*/
void VelCrl2(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t vel)
{
	SetJoggingVel2(CANx, ElmoNum, vel);
...
* @author ACTION
*/
void PosCrl(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t pos1,int32_t pos2)
{
	SendPosCmd(CANx, ElmoNum, pos1, pos2);
//	BeginMotion(CANx, ElmoNum);	
}


void PosCrl2(CAN_TypeDef* CANx, uint8_t ElmoNum,uint8_t posMode,int32_t pos)
{
	SendPosCmd2(CANx, ElmoNum, posMode, pos);
//	BeginMotion(CANx, ElmoNum);	
}
...
* @note：速度正负号代表旋转的方向，大于零为正方向，小于零为负方向
*/
void SetJoggingVel(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t vel1,int32_t vel2)
{
	//第一个数据发送JV命令，第二个数据发送命令值
	uint32_t data[1][2]={
							0x00000000,0x00000000,		//JV
					 };
	uint8_t mbox;
	CanTxMsg TxMessage;
					 
	TxMessage.StdId=SPEEDCONTROL_BASEID + ElmoNum;		// standard identifier=0
	TxMessage.ExtId=SPEEDCONTROL_BASEID + ElmoNum;	  	// extended identifier=StdId
	TxMessage.IDE=CAN_Id_Standard;			 			// type of identifier for the message is Standard
	TxMessage.RTR=CAN_RTR_Data;			 				// the type of frame for the message that will be transmitted
	TxMessage.DLC=8;
...
void SetJoggingVel2(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t vel)
{
	//第一个数据发送JV命令，第二个数据发送命令值
	uint32_t data[1][2]={
...
*/
void SendPosCmd(CAN_TypeDef* CANx, uint8_t ElmoNum,int32_t pos1,int32_t pos2)
{
	uint32_t data[1][2]={
	...
	TxMessage.StdId=POSCONTROL_BASEID+ElmoNum;					 // standard identifier=0
	TxMessage.ExtId=POSCONTROL_BASEID+ElmoNum;					 // extended identifier=StdId
	TxMessage.IDE=CAN_Id_Standard ;			 // type of identifier for the message is Standard
	TxMessage.RTR=CAN_RTR_Data  ;			 // the type of frame for the message that will be transmitted
	TxMessage.DLC=8;
...
void SendPosCmd2(CAN_TypeDef* CANx, uint8_t ElmoNum,uint8_t posMode,int32_t pos)
{
